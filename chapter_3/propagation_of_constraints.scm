  ...
  (define (me request)
    (cond ((eq? request 'I-have-a-value)
          ...
          ((eq? request 'I-lost-my-value)
           (process-forget-value))
          (else 
            (error "Unknown request -- PRODE" request))))
  (connect connector me)
  me)
...
(define (for-each-except exception procedure me-list)
  (define (loop items)
...
; 3.34
;; 乘法约束需要三个连线器，限定其中两个给出第三个值，设定一个值得出另外两个值。乘法约束没有这样的功能。
